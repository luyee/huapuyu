...
/**
 * <p>
 * A JUnit &#64;Rule that can be used to detect and configure a remote Hadoop cluster, or fall back to running locally for a simple functional test.
 * </p>
 * 
 * <p>
 * Example usage
...
public class HadoopSetUp extends TestWatchman {
	...
	/**
	 * Use this factory to run the job only on a remote cluster, and skip all tests if the cluster is not detected.
	 * 
	...
	/**
	 * Detect a cluster with default parameters (job tracker at <code>locahost:9001</code>). Use this factory to run the job only on a remote cluster, and skip all tests if the cluster is not detected.
	 * 
	 * @return a new rule that assumes an existing running cluster
	 */
	...
	/**
	 * Use this factory method to prefer a cluster deployment, but fall back to local if the cluster is not running.
	 * 
	...
	/**
	 * Detect a cluster with default parameters (job tracker at <code>locahost:9001</code>). Use this factory method to prefer a cluster deployment, but fall back to local if the cluster is not running.
	 * 
	 * @return a new rule that prefers an existing running cluster
	 */
