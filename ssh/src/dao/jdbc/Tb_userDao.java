...
public class Tb_userDao extends JdbcDaoSupport implements ITb_userDao {
	...
	public Tb_user getById(int id) {
		Object[] object = new Object[] { id };
		return (Tb_user) getJdbcTemplate().queryForObject(
				"SELECT * FROM tb_user WHERE id = ?", object,
				new BeanPropertyRowMapper(Tb_user.class));
	}

	@SuppressWarnings("unchecked")
	@Transactional(propagation = Propagation.NOT_SUPPORTED, readOnly = true)
	public List getAll() {
		return getJdbcTemplate().query("SELECT * FROM tb_user",
				new BeanPropertyRowMapper(Tb_user.class));
	}

	public void save(Tb_user model) {
		Object[] object = new Object[] { model.getId(), model.getName(),
				model.getPwd() };
		getJdbcTemplate().update(
				"INSERT INTO tb_user (id, name, pwd) VALUES (?, ?, ?)", object);
	}

	@Transactional(isolation = Isolation.READ_COMMITTED)
	public void update(Tb_user model) {
		Object[] object = new Object[] { model.getName(), model.getPwd(),
				model.getId() };
		getJdbcTemplate().update(
				"UPDATE tb_user SET name = ?, pwd = ? WHERE id = ?", object);
	}
}
