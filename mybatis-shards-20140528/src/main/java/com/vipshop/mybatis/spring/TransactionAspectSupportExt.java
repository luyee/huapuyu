 ...
 * 
 * <p>
 * Uses the <b>Strategy</b> design pattern. A {@code PlatformTransactionManager} implementation will perform the actual transaction management, and a {@code TransactionAttributeSource} is used for determining transaction definitions.
 * 
 * <p>
 ...
 */
public abstract class TransactionAspectSupportExt implements BeanFactoryAware, InitializingBean {
	 ...
	 * <p>
	 * To find out about specific transaction characteristics, consider using TransactionSynchronizationManager's {@code isSynchronizationActive()} and/or {@code isActualTransactionActive()} methods.
	 * 
	 ...
	 * @param targetClass
	 *            the target class that we're invoking the method on
	 * @param invocation
	 *            the callback to use for proceeding with the target invocation
	 ...
	 * 
	 * @param method
	 *            the method about to execute
	 * @param targetClass
	 *            the class that the method is being invoked on
	 * @return a TransactionInfo object, whether or not a transaction was created. The {@code hasTransaction()} method on TransactionInfo can be used to tell if there was a transaction created.
	 * @see #getTransactionAttributeSource()
	 * @deprecated in favor of {@link #createTransactionIfNecessary(PlatformTransactionManager, TransactionAttribute, String)}
	 */
	@Deprecated
	...
	private void prepareCommitOrRollback(TransactionInfoWrap txInfoWrap) {
		...
		if (!TransactionSynchronizationManager.isSynchronizationActive() && status.isNewSynchronization()) {
			...
			TransactionDefinition definition = txInfoWrap.getTransactionAttribute();
	...
	private boolean attrRollback(Throwable ex) {
		Map<DataSource, LinkedList<TransactionInfoWrap>> dataSource2TxTree = TransactionHolder.getDataSource2TxTree();
		if (MapUtils.isNotEmpty(dataSource2TxTree)) {
			for (Map.Entry<DataSource, LinkedList<TransactionInfoWrap>> entry : dataSource2TxTree.entrySet()) {
				Iterator<TransactionInfoWrap> it = entry.getValue().descendingIterator();
				...
				while (it.hasNext()) {
					TransactionInfoWrap txInfoWrap = it.next();
					txInfoWrap.getTransactionAttribute().rollbackOn(ex);
				}
			}
		}
		return true;
