...
public class CharacterSet {
	...
	private static void oneSetResponse(String stmt, MySQLFrontConnection c,
			int rs) {
	...
	private static void multiSetResponse(String stmt, MySQLFrontConnection c,
			int rs) {
		...
		for (int i = 1; i < sqlList.length; i++) {
			String sql = new StringBuilder("set ").append(sqlList[i])
					.toString();
			...
			default:
				...
				logger.warn(s.append(c).append(sql).append(" is not executed")
						.toString());
			}
		}
		...
		if (charConnection.startsWith("'") || charConnection.startsWith("`")) {
			charConnection = charConnection.substring(1,
					charConnection.length() - 1);
		}
		...
		if (charConnection.equalsIgnoreCase(charResult)) {
			...
			sb.append("charset is not consistent:[connection=").append(
					charConnection);
	...
	private static void setCharset(String charset, MySQLFrontConnection c) {
