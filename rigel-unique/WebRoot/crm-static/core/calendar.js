 ...
 * past	    默认 true  当设置为false  不可以选择今天以前的日期
 * month    默认 false  当设置为YYYY-MM 后日历只能在设置的月里选择
 */
...
RCalendar.clearCals = function (eles) {
    var source = RCalendar.cals;
    (!eles.length && eles.length != 0) && (eles = [eles]);
...
RCalendar.prototype = {
	...
	init : function () {
		...
		function showBeginCal() {
			...
			var cal = new Calendar({
				"onselect" : function (date) {
					...
					if (config.interval !== false && G(config.endDate).value) {
						...
						var en = me.end;
						var st = date;
						var stDate = new Date(en.getFullYear(), en.getMonth() - months, en.getDate());
						if (stDate.getMonth() + months > en.getMonth() && stDate.getFullYear() == en.getFullYear()) {
						    stDate = new Date(stDate.getFullYear(), stDate.getMonth(), 0);
						}
						if (st < stDate) {
							return false;
						}
					...
					if (date < new Date() && !config.past) {
					    if (me.formatDate(date) == me.formatDate(new Date())) {
					    }
					    else {
						return false;
					    }
					...
					if (config.month) {
					    var str = config.month.split('-'),
						mbegin = new Date(parseInt(str[0], 10), parseInt(str[1], 10) - 1),
						mend = new Date(parseInt(str[0], 10), parseInt(str[1], 10), 0);
					    if (date < mbegin || date > mend) {
						return false;
					    }
					}
			...
			if (G(config.beginDate).value.trim() == '' && config.month) {
			    var s = config.month.split('-');
			    cal.setDate(new Date(parseInt(s[0], 10), parseInt(s[1], 10) - 1))
			}
			...
			var cal = new Calendar({
				"onselect" : function (date) {
					...
					if (config.interval !== false && G(config.beginDate).value) {
						var months = parseInt(config.interval, 10);
						var st = me.begin;
						var enDate = new Date(st.getFullYear(), st.getMonth() + months, st.getDate());
						if (enDate.getMonth() - months > st.getMonth() && enDate.getFullYear() == st.getFullYear()) {
						    enDate = new Date(enDate.getFullYear(), enDate.getMonth(), 0);
						}
						if (enDate < date) {
							return false;
						}
