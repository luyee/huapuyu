...
function initTable(table, column, config)
{
	...
	table.setRowLight = Table.setRowLight;
	table.flush();
...
var Table =
{
	...
	dragMouseUp: function (event)
	{
		...
		document.onmouseup = "";
		table.setRowLight(table.lightIndex, true);
	},
	/**
	 * 点击表格控件的单选列表头的处理
	 * @public
	...
	setRowLight : function (row, toggle) {
	    if (!row && row != 0) {
		return;
	    }
	    var list = this.tagName.toLowerCase() == "table" ? this.getElementsByTagName("td") : this.getChildren("div"),
		column = this.column,
		index = row * column;

	    for (var i = 0; i < column; i++) {
		if (toggle) {
		    list[i + index].className ? list[i + index].className += ' light' : list[i + index].className = ' light';
		}
		else {
		    list[i + index].className = list[i + index].className.replace(/ light/g, '');
		}
	    }
	},

	toggleLightRow : function (e) {
	    e = getEvent(e);
	    if (e.target !== this) {
		return;
	    }
	    
	    var table = this.nodeName.toLowerCase() == 'td' ? this.parentNode.parentNode.parentNode : this.parentNode;
		lightIndex = table.lightIndex,
		column = table.column,
		list = table.tagName.toLowerCase() == "table" ? table.getElementsByTagName("td") : table.getChildren("div");
	    
	    if (lightIndex) {
		table.setRowLight(lightIndex);
	    }
	    for (var i = column; i < list.length; i++) {
		if (list[i] == this) {
		    i = parseInt(i / column);  
		    break;
		}
	    ...
	    if (i != lightIndex) {
		lightIndex = i;
		table.setRowLight(lightIndex, true);
		
	    }
	    else {
		lightIndex = null;
	    }
	    table.lightIndex = lightIndex;
	},
	...
	flush: function ()
	{
		...
		for (var i = column, item; item = G(list[i]); i++)
		{
			...
			if (!item.onclick) {
			    item.onclick = Table.toggleLightRow; 
			}
