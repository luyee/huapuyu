 ...
 */
if(typeof log == "undefined") var log = null;

function initTable(table, column, config)
{
	var el, i, width;
  log && log.info('initTable start:' + log.callId);
	table = G(table);
  log && log.info('search parentNode display:' + log.callId);
	var parents = [];
	for (var parent = G(table.parentNode); parent && parent.tagName.toLowerCase() != 'body'; parent = G(parent.parentNode))
	{
    log && log.info(parent.tagName + parent.css('display') + ':' + log.callId);
  ...
  log && log.info('calculate width start:' + log.callId);
	list = table.getChildren('div');
	// 计算整个表格的宽度
	for (i = 0, width = 0; i < column; i++)
	{
		el = G(list[i]);
    log && log.info(i + el.offsetWidth + ':' + log.callId);
		el.offsetWidth;
  ...
  log && log.info('calculate width end:' + log.callId);
	table.css('width',
		(width + table.css('borderLeftWidth').toNumber()
		+ table.css('borderRightWidth').toNumber()) + "px");
	if (config)
	{
    log && log.info('config start:' + log.callId);
    ...
    log && log.info('config end:' + log.callId);
	}
	table.config = config;
	table.column = column;
  ...
  log && log.info('function set end and flush start:' + log.callId);

  try {
	  table.flush();
  } catch (ex) {
    log && log.info('table flush error:' + log.callId + ';message:' + ex.message);
  }
  log && log.info('flush end:' + log.callId);
