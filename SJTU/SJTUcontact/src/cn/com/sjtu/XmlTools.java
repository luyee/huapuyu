...
public class XmlTools {
	...
	public XmlTools() {
		...
		this.thirdChild = Constants.CREATEDATE;
		this.forthChild = Constants.MODIFIEDDATE;
	...
	public int writeXml(String fileName,List<User> listUser,boolean code) throws IllegalArgumentException, IllegalStateException, IOException{
		...
		for (User user : listUser) {
			...
			serializer.attribute("", Constants.GROUPID, user.getGroupnum()+"");
	...
	public List<User> readXml(String fileName2) throws FileNotFoundException, IOException, ParserConfigurationException, SAXException {
	...
	private class MyXmlHandler extends DefaultHandler{
	    ...
	    public void startElement(String uri, String localName, String qName, Attributes attrs) throws SAXException {
	    	tagName = localName;
	    	if(tagName.equals(Constants.CONTACT)) isCode = Boolean.parseBoolean(attrs.getValue(Constants.CODE));
		...
		public void characters(char[] ch, int start, int length)
			...
			} else if (tagName.equals(Constants.CREATEDATE)) {
				user.setCreatedDate(changeCharToString(ch, start, length));
			} else if (tagName.equals(Constants.MODIFIEDDATE)) {
				user.setModifiedDate(changeCharToString(ch, start, length));
	    ...
	    private String changeCharToString(char[] ch, int start, int length){
	    	if(isCode){
	    		return Tools.deCode(new String(ch,start,length));
	    	}else{
