...
begin
  ...
  //初始化数据
  house := THouse.Create;
  ShowMessage(house.ToString);
  house.MappingData(Self.id);

  ShowMessage(house.ToString);


  secondHandHouse := TSecondHandHouse.Create;
  //secondHandHouse.Id := fieldList.FieldByName('ID').AsInteger;
...
procedure TFrmEdit.SetId(const id: Integer);
begin
  Self.id := id;
end;

procedure TFrmEdit.Validate(Sender: TWinControl);
begin
