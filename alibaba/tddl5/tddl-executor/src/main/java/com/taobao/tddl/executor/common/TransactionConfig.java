...
public class TransactionConfig implements Cloneable {
     ...
     * stringent ones first. All environment level defaults were applied by the
    ...
    public Durability getDurabilityFromSync(/* EnvironmentImpl envImpl */) {
         ...
         */
        /*
         * if (envImpl.isReplicated()) { return Durability.COMMIT_NO_SYNC; }
         * else {
         */
        return Durability.COMMIT_SYNC;
        // }
     ...
     * Durability or explicit sync configuration. This method is used to avoid a
     * mixed mode exception, since the existing config may be in either mode.
     */
    ...
    /**
     * Configures this transaction to have serializable (degree 3) isolation. By
     * setting serializable isolation, phantoms will be prevented.
     * <p>
     * By default a transaction provides Repeatable Read isolation;
     * {@link EnvironmentConfig#setTxnSerializableIsolation} may be called to
     * override the default. If the environment is configured for serializable
