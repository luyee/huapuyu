...
public class BloomFilter<E> implements Serializable {
	...
	static final Charset charset = Charset.forName("UTF-8"); // encoding used for storing hash
																// values as strings

	static final String hashName = "MD5"; // MD5 gives good enough accuracy in most circumstances.
	...
	public BloomFilter(double falsePositiveProbability, int expectedNumberOfElements) {
		this(Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2), // c = k
																							// /
																							// ln(2)
	 ...
	 *            specifies how many elements have been inserted into the <code>filterData</code>
	 *            BitSet.
	 * @param filterData
	 ...
	 * 4-byte int's and store them in an array. The digest function is called until the required
	 * number of int's are produced. For each call to digest a salt is prepended to the data. The
	 ...
	 * <br />
	 * The value returned by this method is the <i>expected</i> rate of false positives, assuming
	 * the number of inserted elements equals the number of expected elements. If the number of
	 * elements in the Bloom filter is less than the expected value, the true probability of false
	...
	/**
	 * Calculate the probability of a false positive given the specified number of inserted
	 * elements.
	 * 
	...
	/**
	 * Get the current probability of a false positive. The probability is calculated from the size
	 * of the Bloom filter and the current number of elements added to it.
	 * 
	...
	/**
	 * Returns the number of bits in the Bloom filter. Use count() to retrieve the number of
	 * inserted elements.
	 * 
	...
	/**
	 * Returns the expected number of elements to be inserted into the filter. This value is the
	 * same value as the one passed to the constructor.
	 * 
	 ...
	 * been inserted and the length of the Bloom filter. See also getExpectedBitsPerElement().
	 * 
