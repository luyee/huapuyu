...
import com.anders.ethan.log.dapper.client.classic.LocalAsySendHolder;
import com.anders.ethan.log.dapper.client.classic.LocalAsySendImpl;
import com.anders.ethan.log.dapper.common.Annotation;
...
public class TraceGenerater implements TraceGenerator, ITraceFactory {
	...
	private static Logger logger = LoggerFactory
			.getLogger(TraceGenerater.class);
	...
	// 构建Span，参数通过上游接口传递过来
	@Override
	public Span genSpan(String appName, String traceId, String pid, String id,
	...
	public boolean isSample() {
		return (asySendService != null && asySendService.isReady())
				&& sampler.isSample();
	}
	...
	public void asyCatchUnExceptionLogSpan(Throwable throwable, String detail) {
		...
		span.setServiceType(TraceGenerater.getTracer()
				.getServiceTypeThreadLocal());
		span.setLevel("ERROR");
		...
		if (stackTrace.length == 0) {
			...
			span.setMethod(lastElem.getClassName() + ":"
					+ lastElem.getMethodName());
			span.setLogIdentity(lastElem.getClassName() + ":"
					+ lastElem.getMethodName() + ":" + lastElem.getLineNumber());
		}
