...
public class TraceGenFactory {
    ...
    private static ITraceFactory getTraceFactory() {
        ...
        switch (INIT_STATE) {
            case ONGOING_INIT:
                throw new RuntimeException("invalid state, state is " + ONGOING_INIT);
            case NOT_FOUND_IMP:
                return DEFAULT_FACTORY;
            case INIT_SUCCESS:
                return StaticTraceGenBinder.SINGLETON().getTraceFactory();
            case INVALID_VERSION:
                throw new IllegalArgumentException("incompatible version, required is " +
                        StaticTraceGenBinder.SINGLETON().getRequiredVersion() + ", compatible is " +
...
//        PatrolUtils.registerAppConfigMBean(AppTraceConfig.getLocalConfig());
    ...
    private static void reportActualBinder(Set<URL> multiImplSet) {
        if (multiImplSet.size() > 1) {
            System.err.println("Actual Binder is [" + StaticTraceGenBinder.SINGLETON().getTraceGenStr() + "]");
        }
    }
    ...
    private static Set<URL> findImpossibleTraceGenImpl() {
        ...
        try {
            ...
            if (classLoader != null) {
                urlEnumeration = classLoader.getResources(TRACE_GEN_IMPL_CLASS_NAME);
            } else {
                urlEnumeration = ClassLoader.getSystemResources(TRACE_GEN_IMPL_CLASS_NAME);
            }
